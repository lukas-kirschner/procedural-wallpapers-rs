...
static void fill_background(Perlin<BitDepth> const &perlin) {
    for (uint32_t x = 0; x < bytes->width; ++x) {
        for (uint32_t y = 0; y < bytes->height; ++y) {
            uint8_t const greyval =
                    ((perlin.fractal(x, y, 0.004f, 5) + 1.0f) / 2.0f) * (double) (PIXEL_BACKGROUND_MAX_GREYVALUE - 1);
 ...
 */
template<typename BitDepth>
static double get_star_likeliness(size_t x, size_t y, Perlin<BitDepth> const &perlin) {
    float const noise_amount = perlin.fractal(x, y, 0.002f, 5);
...
void draw() {//TODO fill background with noise?
    ...
    fill_background(myPerlin);
    myPerlin.generate_noise();
    for (int x = 0; x < bytes->width; ++x) {
        for (int y = 0; y < bytes->height; ++y) {
            if (!(rand() % 12)) {
                double const likeliness = get_star_likeliness(x, y, myPerlin);
