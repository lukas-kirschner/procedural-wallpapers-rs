...
impl Tangles {
    ...
    fn draw_rectangle(&self, rng: &mut impl Rng, img: &mut RgbImage, x0: usize, y0: usize, w: usize, h: usize, base_color: [u8; 3]) -> Result<(), String> {
        let mut perlin = Perlin::new(w, h);
        perlin.regenerate_noise(rng);
        for x in x0..(x0 + w) {
            for y in y0..(y0 + h) {
                let noise_shade: f64 = 200.0 + (perlin.fractal((x - x0) as f64, (y - y0) as f64, 0.002, 6)? * 55.0);
    ...
    fn draw_random_rectangle(&self, rng: &mut impl Rng, img: &mut RgbImage, maxsize: usize, base_color: [u8; 3]) -> Result<(), String> {
...
impl<R: Rng> Algorithm<R> for Tangles {
    fn build(&mut self, rng: &mut R, img: &mut RgbImage) -> Result<(), String> {
        ...
        let num_rectangles = min(max(16, img.width() * img.height() / 80000), 255);
        for i in 0..num_rectangles {
            let maxsize = (img.height() - i * img.height() / 40).try_into().expect("maxsize out of bounds");
            let base_color = [
                base_r - i as u8 * 5,
                base_g - i as u8 * 5,
                base_b - i as u8 * 5,
